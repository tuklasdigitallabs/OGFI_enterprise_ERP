...
public sealed class StockConsequenceProcessor(ProcurementDbContext procurementDb, GoodsReceiptPostedConsumer inventoryConsumer, IStockConsequenceAttemptHook attemptHook, TimeProvider timeProvider, ILogger<StockConsequenceProcessor> logger)
{
    public async Task ProcessTenantAsync(Guid tenantId, CancellationToken cancellationToken)
    {
        var pending = await procurementDb.OutboxMessages.Where(x => x.TenantId == tenantId && x.ProcessedAtUtc == null && x.Type == "Procurement.GoodsReceiptPosted" && x.SchemaVersion == 1).OrderBy(x => x.OccurredAtUtc).Take(50).ToListAsync(cancellationToken);
        foreach (var message in pending)
        {
            ...
            try
            {
                ...
                var payload = JsonSerializer.Deserialize<GoodsReceiptPostedV1>(message.Payload) ?? throw new InventoryRuleException("INVENTORY.EVENT.INVALID", "GoodsReceiptPosted payload is empty.");
                if (payload.TenantId != tenantId || payload.EventId != message.Id) throw new InventoryRuleException("INVENTORY.EVENT.TENANT_MISMATCH", "GoodsReceiptPosted envelope identity does not match its payload.");
                await inventoryConsumer.ApplyAsync(payload, cancellationToken);
                message.LastError = null; message.ProcessedAtUtc = timeProvider.GetUtcNow();
                await procurementDb.SaveChangesAsync(cancellationToken);
            }
            catch (InventoryRuleException ex)
            {
                message.LastError = ex.Code; message.ProcessedAtUtc = timeProvider.GetUtcNow();
                await procurementDb.SaveChangesAsync(cancellationToken);
            ...
            {
                message.LastError = "INVENTORY.EVENT.INVALID_JSON"; message.ProcessedAtUtc = timeProvider.GetUtcNow();
                await procurementDb.SaveChangesAsync(cancellationToken);
